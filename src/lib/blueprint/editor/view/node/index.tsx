...
export function NodeView(props: Props) {
  ...
  const outerSize = props.data.nest
    ? computeNodeSizeByDefine(category)
          ...
          return (
            <div
              ...
              style={
                ...
                `,
                  borderBottom:
                    ci !== category.length - 1 && cate.label
                      ? style.category.borderBottomNotLast
                      : "",
                  marginBottom: cate.halfline
                    ? `-${computeBlockHeight(cate)}px`
                    : 0,
            ...
            >
              ...
              {(cate.extend?.activate
                ? [...cate.content, ...(cate.extend.content || [])]
                : cate.content
              ).map((item, ii) => {
                if (item.type !== "output" && item.type !== "control") {
                  const inputPort = inputs[item.name];
                  if (inputPort) {
                    inputPort.socket.dataType =
                      item.dataType || StarmapDataType.UNKNOW;
                ...
                if (item.type !== "input" && item.type !== "control") {
                  ...
                  if (outputPort) {
                    outputPort.socket.dataType =
                      item.anotherDataType ||
                      item.dataType ||
                      StarmapDataType.UNKNOW;
                    outputPort.socket.flowType =
                      item.anotherFlowType || item.flowType;
                  }
                }
                return (
                  <div
                    ...
                    style={{
                      ...
                      textAlign:
                        item.type === "input" || item.type === "control"
                          ? "left"
                          : item.type === "both"
                            ? "center"
                            : "right",
                      padding:
                        item.type === "both"
                          ? ""
                          : `0 ${!cate.content.includes(item as any) ? style.blockLine.extendIndent : style.blockLine.indent}`,
                    }}
                  >
                    {item.label ? (
                      <span
                        className="socket-label"
                        style={{ fontSize: style.blockLine.fontSize }}
                      >
                        {item.label}
                      </span>
                    ) : (
        ...
        <>
          ...
          <div
            ...
            onPointerDown={(event: React.PointerEvent) => {
              ...
              const { width, height } = computeGroupSizeByDefine(
                category,
                nest,
              );
              (props.emit as (props: NodeScaleContext) => void)({
                type: "nsstart",
                data: {
                  id: props.data.id,
                  constraint: { min: { width, height } },
                  event,
                },
              });
            }}
