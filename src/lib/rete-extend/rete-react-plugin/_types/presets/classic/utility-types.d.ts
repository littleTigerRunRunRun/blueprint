...
type UnionToTuple<T> = UnionToIntersection<T extends never ? never : (t: T) => T> extends (_: never) => infer W ? [...UnionToTuple<StrictExclude<T, W>>, W] : [];
type TupleToObject<T extends readonly any[], Rest extends object> = {
    [K in keyof T]: React.JSXElementConstructor<{
        data: T[K];
    } & Rest>;
}[number];
export type AcceptComponent<T, Rest extends object = {}> = TupleToObject<UnionToTuple<T>, Rest>;
export {};
