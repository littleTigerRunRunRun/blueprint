...
function getRenderer(props) {
  ...
  return {
    mount: function (element, container) {
      return ReactDOM__namespace.render(element, getWrapper(container));
    },
    unmount: function unmount(container) {
      ReactDOM__namespace.unmountComponentAtNode(getWrapper(container));
      removeWrapper(container);
    }
  };
}
...
var _excluded = ["name", "emit", "nodeId", "side", "inout", "socketKey", "payload"];
