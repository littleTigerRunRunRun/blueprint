...
export async function createEditor(container: HTMLElement) {
  ...
  const process = () => {
    ...
    editor
      .getNodes()
      .filter((n) => n instanceof UniNode)
      .forEach((n) => {
        engine.fetch(n.id)
      })
  }

  const createUniNode = getCreateUniNode<'number', number>({
    updateOutputControls: (ids) => {
      for (const id of ids) { area.update("control", id) }
    }
  })
  ...
  const createNumber = () => {
    return createUniNode({
      ...
      outputs: {
        value: { label: '参数', control: { type: 'number', initial: 1, change: process } }
      },
      controls: {},
      dataOperation: (inputs, self): { value: number } => {
        return {
          value: (self.controls.value.value as number) || 0
        }
      }
    })
  }
  ...
  const createAdd = () => {
    return createUniNode({
      ...
      height: 200,
      inputs: {
        param1: { label: '参数一', control: { type: 'number', initial: 0, change: process } },
        param2: { label: '参数二', control: { type: 'number', initial: 0, change: process } }
      },
      outputs: {
        result: { label: '结果', control: { type: 'number', readonly: true } }
      },
      controls: {},
      dataOperation: (inputs, self): { result: number} => {
  ...
  const a = createNumber()
  const b = createNumber()
  const c = createAdd()
