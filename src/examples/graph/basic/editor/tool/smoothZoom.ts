...
export class SmoothZoom extends Zoom {
  ...
  private area:AreaPlugin<Schemes, MyAreaExtra>|null = null
  setAnime(duration: number, easing: string, area: AreaPlugin<Schemes, MyAreaExtra>) {
...
export const attachSmoothArea = (area:AreaPlugin<Schemes, MyAreaExtra>) => {
  const sz = new SmoothZoom(0.5, {
    dblclick: (_delta:number) => 0
  })
  sz.setAnime(200, "cubicBezier(.45,.91,.49,.98)", area)
  area.area.setZoomHandler(sz);
}
