...
export type NodeScaleContext = Area2D<Schemes> | Root<Schemes> | nodeScaleEvent

export class NodeScalablePlugin<S> extends Scope<
  nodeScaleEvent,
  Area2DInherited<Schemes, nodeScaleEvent>
> {
  ...
  setParent(scope: Scope<Area2D<Schemes>, [Root<Schemes>]>): void {
    ...
    this.addPipe((context) => {
      if (context.type === 'nsstart') {
        ...
        if (this.nsid) {
          const delta = {
            x: context.data.event.pageX - this.nsStartPosition.x,
            y: context.data.event.pageY - this.nsStartPosition.y
          }
          this.nsStartPosition.x = context.data.event.pageX
          this.nsStartPosition.y = context.data.event.pageY
